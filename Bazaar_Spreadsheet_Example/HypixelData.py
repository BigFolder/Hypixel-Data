...
def checkCrashData(newData):
    ...
    # Create File if it does not already exist.
        ...
        create = open("crashData.txt", 'w')
        create.close()
        checkCrashData(newData)


'''
Saves the most the most recent product data into a file to be used to check for market crashes 
This can be made into some data structure in your code instead to nullify the use of this text file.
...
def gather_bazaar_data(API_KEY, fileList):
    ...
    for productID in data['products']:
        # Both of these are boolean checks to ensure a product is valid data.
